-- Auto Quest Script with Task Monitoring
local CONFIG = {
    ...
    TaskCheckInterval = 1,     -- How often to check task progress (seconds)
    CompletionDelay = 5        -- Wait after reaching 100% (seconds)
}
...
local function GetQuests()
    local quests = {
        ...
        eggs = {},
        collections = {},
        tasks = {}  -- New task category
    }
    
    -- Existing quest detection
    ...
    -- New task detection
    local tasksFolder = ScreenGui.Competitive.Frame.Content.Tasks
    if tasksFolder then
            ...
            if template.Name == "Template" then
                ...
                if content then
                    ...
                    if bar then
                        ...
                        if label then
                            local percentText = label.Text:match("%d+")
                            local percent = tonumber(percentText) or 0
                            
                            table.insert(quests.tasks, {
                                template = template,
                                label = label,
                                percent = percent
                            })
...
local function RunTaskQuests()
    local quests = GetQuests().tasks
    if #quests == 0 then return false end

    print("Starting", #quests, "task quest(s)")

    for _, task in ipairs(quests) do
        print("Monitoring task:", task.label.Text)
        
        -- Wait until 100% completion
        while task.percent < 100 do
            task.percent = tonumber(task.label.Text:match("%d+")) or 0
            print("Current progress:", task.percent.."%")
            wait(CONFIG.TaskCheckInterval)
        end

        print("Task reached 100% - waiting", CONFIG.CompletionDelay, "seconds")
        wait(CONFIG.CompletionDelay)
    end

    print("All task quests completed!")
    return true
end
...
local function HatchInfinityEgg()
    ...
    -- Monitor for new quests including tasks
    local startTime = os.time()
    while #GetQuests().bubbles == 0 and #GetQuests().eggs == 0 
        and #GetQuests().collections == 0 and #GetQuests().tasks == 0 do
...
while true do
    ...
    elseif RunCollectionQuests() then
        continue
    elseif RunTaskQuests() then  -- New task check
        continue
    else
        HatchInfinityEgg()
    end
